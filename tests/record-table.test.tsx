...
// mocks to satisfy imported hooks and componentsvi.mock('@/hooks/use-app-setting', () => ({ useAppSetting: () => [[], vi.fn()] }));
...
describe('RecordTable sanity', () => {
  it('does not crash when collection prop starts undefined then becomes defined', () => {
    ...
    // update props to a minimal collection    rerender(
      <BrowserRouter>
        <RecordTable data={[]} collection={{ name: 'foo', fields: [] } as any} loading={false} />
      </BrowserRouter>
    );

    // expect something in dom (table wrapper)    expect(document.body).toBeTruthy();
  });
});
