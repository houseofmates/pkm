...
describe('SchemaService', () => {
  ...
  it('should generate a Zod schema for a dynamic table and validate records', () => {
    // 1. define the structure of our dynamic table    const userProfileTable: FieldInstance[] = [
      { name: 'username', type: 'text' },
      { name: 'age', type: 'number' },
      { name: 'bio', type: 'text' },
    ];

    // 2. generate the zod schema from this structure    const UserProfileSchema = schemaService.generateRecordSchema(userProfileTable);

    // 3. define a valid record and an invalid record    const validUser = {
      username: 'john_doe',
      age: 30,
    ...
    // 4. validate the records and assert the results    const validResult = UserProfileSchema.safeParse(validUser);
    const invalidResult = UserProfileSchema.safeParse(invalidUser);

    // the valid user should pass validation    expect(validResult.success).toBe(true);
    if (validResult.success) {
      expect(validResult.data).toEqual(validUser);
    }

    // the invalid user should fail validation    expect(invalidResult.success).toBe(false);
  });
  ...
  it('should throw an error for unregistered field types', () => {
    ...
    // expect the schema generation to throw an error because 'notatype' is not a known type    expect(() => {
