...
/*
  ...
  Usage:
    NOCOBASE_URL=http://localhost:4100/api ADMIN_API_KEY="Bearer <token>" node scripts/normalize-collections.cjs [--dry-run]
...
if (!base || !apiKey) {
  ...
  console.error('Example: NOCOBASE_URL=http://localhost:4100/api ADMIN_API_KEY="Bearer ..." node scripts/normalize-collections.cjs --dry-run');
  process.exit(2);
}
  ...
  try {
    ...
    for (const { col, desired } of updates) {
      ...
      const candidates = [
        ...
        `${baseUrl}/collections:update?filterByTk=${encodeURIComponent(col.id)}`
      ];
      ...
      for (const url of candidates) {
        try {
          tried.push(url);
          const res = await axios.post(url, { title: desired, hidden: true }, { headers });
          console.log(`  Updated ${col.name} via ${url} -> status ${res.status}`);
          success = true;
          break;
        } catch (err) {
      ...
      if (!success) {
        console.error(`  All update attempts failed for ${col.name}. Tried:`, tried);
      }
    }
