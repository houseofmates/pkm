...
(async () => {
  ...
  // intercept llm generate calls and return a deterministic response  await page.route('**/api/generate', async (route, request) => {
  ...
  // stub only the nocobase endpoints used during app init to avoid 401 clearing the token  await page.route('**/api/headmates:list*', (route) => {
  ...
  // also stub direct backend origins used in some environments  await page.route('http://localhost:4100/**', (route) => {
  ...
  // intercept external nocobase host used by dev proxy to avoid remote 401 responses  await page.route('https://db.houseofmates.space/api/**', (route) => {
    return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ data: [] }) });
  });

  // auto-accept prompt with the user's question  page.once('dialog', async (dialog) => {
  ...
  // log client console + page errors to help debug why toolbar may not mount  page.on('console', msg => console.log(`[browser console:${msg.type()}] ${msg.text()}`));
  page.on('pageerror', err => console.log('[browser pageerror]', err));

  // log network requests/responses for /api/* to find failing endpoints  page.on('request', (req) => {
  ...
  // ensure app treats the session as an authenticated private app  await page.addInitScript(() => {
    try {
      localStorage.setItem('nocobase_token', 'test-token');
      // lightweight drawing config so the page shows a title if read      localStorage.setItem('drawing-config-test-ai', JSON.stringify({ title: 'test drawing' }));
    } catch (e) {
      // ignore    }
  });

  // navigate in private mode so /drawings route is available  await page.goto(`${base}/drawings/test-ai?mode=private`, { waitUntil: 'networkidle' });

  // dump a small snapshot of the dom body for debugging if toolbar doesn't appear  const bodyHtml = await page.locator('body').innerHTML();
  ...
  // wait for reply to appear in the chat panel  await page.waitForSelector('text=mocked canvas answer', { timeout: 8000 });

  // validations against intercepted payload  if (!captured) {
