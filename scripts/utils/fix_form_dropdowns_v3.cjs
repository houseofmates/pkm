...
// identify the block to replaceconst oldBlockStart = "{field.type === 'dropdown' && (";
const oldBlockEnd = ")}";

// since there might be multiple field.type === 'dropdown' checks, we need to find the one inside fields.map in formbuilderconst formBuilderStart = "export function FormBuilder";
const fbContent = content.slice(content.indexOf(formBuilderStart));

// find the dropdown check inside formbuilderconst dropdownStartIdx = fbContent.indexOf("{field.type === 'dropdown' && (");
// find the end of this block. it's a bit tricky with nested braces.// but looking at the file, it's followed by </div> and then map close.
...
// use a more aggressive approach: find the exact old block by looking for unique strings inside itconst searchPattern = /\{field\.type === 'dropdown' && \([\s\S]*?className="text-white\/30 hover:bg-white\/10 font-bold uppercase hover:bg-\[var\(--primary\)\]\/20"[\s\S]*?\)\}/;
...
// re-read the file to get exact content of the old blockconst start = fbContent.indexOf("{field.type === 'dropdown' && (");
