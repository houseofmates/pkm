          ...
          <CollapsibleSection title="emotions" isOpen={openSection === 'emotions'} onToggle={() => setOpenSection(openSection === 'emotions' ? null : 'emotions')} panelId="emotions-panel">
            <div className="flex flex-wrap gap-2">
              {EMOTIONS.map((emotion) => (
                <Button key={emotion} variant={entry.emotions.includes(emotion) ? 'default' : 'outline'} size="sm" onClick={() => toggleEmotion(emotion)} className="border-amber-800" aria-pressed={entry.emotions.includes(emotion)}>
                  {emotion}
                </Button>
              ))}
            </div>
          ...
          <CollapsibleSection title="activities" isOpen={openSection === 'activities'} onToggle={() => setOpenSection(openSection === 'activities' ? null : 'activities')} panelId="activities-panel">
            <div className="flex flex-wrap gap-2">
              {ACTIVITIES.map((activity) => (
                <Button key={activity} variant={entry.activities.includes(activity) ? 'default' : 'outline'} size="sm" onClick={() => toggleActivity(activity)} className="border-amber-800" aria-pressed={entry.activities.includes(activity)}>
                  {activity}
                </Button>
              ))}
            </div>
