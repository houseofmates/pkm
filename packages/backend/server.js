...
app.get('/api/ai/pieces/status', requireAuth, async (req, res) => {
  try {
    ...
    res.json({
      connected,
      mcpUrl: process.env.PIECES_MCP_URL || process.env.PIECES_MCP_LOCAL_URL || `http://${process.env.OLLAMA_LOCAL_IP || '192.168.4.233'}:39301/model_context_protocol/2025-03-26/mcp`
    });
  } catch (err) {
