...
async function main() {
  ...
  // ── health check ─────────────────────────────────────────  app.get('/health', (_req, res) => {
    res.json({ status: 'ok', service: 'pkm-weave', uptime: process.uptime() });
  });

  // ── semantic weave ───────────────────────────────────────  console.log('[weave-server] initializing semantic weave...');
  const weaveCtx = await initSemanticWeave(config);
  app.use('/weave', createWeaveRouter(weaveCtx));

  // ── autonomous gardener ──────────────────────────────────  console.log('[weave-server] initializing autonomous gardener...');
  ...
  // ── start server ─────────────────────────────────────────  app.listen(config.port, () => {
