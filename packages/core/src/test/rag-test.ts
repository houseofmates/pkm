...
async function testHermesPromptGeneration(): Promise<TestResult> {
  const start = Date.now();
  const name = 'hermes rag prompt generation';
...
async function testAiFieldGeneration(): Promise<TestResult> {
  ...
  try {
    ...
    const collectionsRes: any = await nocobaseClient.listCollections();
    ...
    const recordsRes: any = await nocobaseClient.listRecords(testCollection, { pageSize: 1 });
