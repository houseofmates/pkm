...
// theme color for sync toastsconst SYNC_THEME_COLOR = '#f6b012';

// single source of truth for all 15 user collectionsconst HARDCODED_COLLECTIONS = [
...
class DataService {
  ...
  private initializeSocket(): void {
    ...
    try {
      ...
      // listen for data updates from server (triggered by other devices)      this.socket.on('data_update', (payload) => {
  ...
  private handleDataUpdate(payload: any): void {
    ...
    switch (type) {
      case 'activity_logged':
        // refresh gamification state from server        this.refreshGamificationState();
        break;
      case 'gamification_updated':
        // refresh gamification state        this.refreshGamificationState();
        break;
      case 'collections_updated':
        // refresh collections        this.syncTables();
        break;
      default:
        secureLogger.info('[DataSync] unknown update type:', type);
    }

    // show subtle sync confirmation toast (only if from another device)    if (source !== 'self') {
      toast.success(
        'system synced',
        {
          style: {
  ...
  public async createTable(name: string, fields: FieldInstance[]): Promise<void> {
    ...
    // after creating, trigger a sync to refresh the ui and cache    await this.syncTables();
    
    // broadcast update to other devices    this.emitDataUpdate('collections_updated', { collection_name: name });
  }
  ...
  public async syncTables(): Promise<void> {
    // 1. load from cache and update ui immediately for instant load    try {
    ...
    // 2. fetch fresh data from the network    try {
      const response = await api.listCollections({ appends: ['fields'] });
      // handle both array and object-with-data-array responses      const freshCollections = Array.isArray(response.data)
        ? response.data
      ...
      // filter out system collections and i18n template entries      const SYSTEM_NAMES = new Set([
      ...
      // merge with hardcoded collections that may be missing from api response      const existingNames = new Set(userCollections.map((c: any) => (c.name || '').toLowerCase()));
      const missingHardcoded = HARDCODED_COLLECTIONS
        .filter(name => !existingNames.has(name.toLowerCase()))
        .map(name => ({ name, title: name, fields: [] }));
      
      // deduplicate: if api returned a collection with a slightly different name (e.g. hygiene-log vs hygiene_log), skip the hardcoded one      const normalizedNames = new Set(userCollections.map((c: any) => (c.name || '').toLowerCase().replace(/[-_]/g, '')));
      ...
      // 3. update the local cache with merged data      await localDbService.saveCollections(mergedCollections);

      // 4. update the ui (zustand store) with merged data      useCollectionsStore.getState().setCollections(mergedCollections as any);
    } catch (error) {
