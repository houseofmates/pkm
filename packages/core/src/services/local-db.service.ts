...
// define the structure of our database using the dbschema interfaceinterface PkmDbSchema extends DBSchema {
  // 'collections' is an object store (like a table).  // 'key' is the type of the primary key (in this case, the collection name).  // 'value' is the type of the data stored.  collections: {
    key: string;
...
class LocalDbService {
  ...
  constructor() {
    this.dbPromise = openDB<PkmDbSchema>(DB_NAME, DB_VERSION, {
      upgrade(db, _oldVersion) {
        // this function runs if the database doesn't exist or the version has changed.        // we create our object stores here.        if (!db.objectStoreNames.contains('collections')) {
        ...
        if (!db.objectStoreNames.contains('oplog')) {
          ...
          // for 'by-synced', index on a synthetic property if needed, but since we can't easily add synthetic,          // let's index on 'synced' assuming modern idb supports boolean,          // or we can handle it via a boolean index since most modern browsers do support it.          oplogStore.createIndex('by-synced', 'synced');
        }
      },
    });
  }
  ...
  public async saveCollections(collections: any[]): Promise<void> {
    ...
    for (let i = 0; i < collections.length; i += CHUNK_SIZE) {
      ...
      // yield to the event loop between chunks to prevent ui lockup      if (i + CHUNK_SIZE < collections.length) {
  ...
  public async saveOplogBatch(entries: OpLogEntry[]): Promise<void> {
    ...
    for (let i = 0; i < entries.length; i += CHUNK_SIZE) {
      ...
      // yield to the event loop between chunks to prevent ui lockup      if (i + CHUNK_SIZE < entries.length) {
        await new Promise(resolve => setTimeout(resolve, 0));
      }
    }
  ...
  public async getUnsyncedOplog(drawingId?: string): Promise<OpLogEntry[]> {
    ...
    // using the 'by-synced' index to quickly find unsynced items.    // if the index expects a boolean:    const unsyncedCursor = await db.getAllFromIndex('oplog', 'by-synced', IDBKeyRange.only(false));
...
// export a singleton instance of the serviceexport const localDbService = new LocalDbService();