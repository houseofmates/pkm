// offline service for apk - caches recent records and queues changes for sync// designed for ~200 records max to stay lightweight on mobile
...
class OfflineService {
  ...
  // cache a record for offline access  cacheRecord(collection: string, id: string, data: any) {
    ...
    if (existingIndex >= 0) {
      ...
      // enforce max cache size with lru eviction      if (this.cache.records.length >= MAX_CACHED_RECORDS) {
        this.evictLRU();
      }
  ...
  // get cached record if available  getCachedRecord(collection: string, id: string): any | null {
  ...
  // get all cached records for a collection  getCachedCollection(collection: string): any[] {
    const records = this.cache.records
      .filter(r => r.collection === collection)
      .sort((a, b) => b.accessedAt - a.accessedAt); // most recent first
    
    // update access times    records.forEach(r => r.accessedAt = Date.now());
    this.saveToStorage();
    
    return records.map(r => r.data);
  }

  // queue a change for when back online  queueChange(action: 'create' | 'update' | 'delete', collection: string, data: any) {
    ...
    // try to sync immediately if online    if (this.isOnline && !this.syncInProgress) {
      this.processQueue();
    }
  ...
  // get pending changes count  getPendingChangesCount(): number {
    return this.queue.length;
  }

  // get pending changes for display  getPendingChanges(): QueuedChange[] {
    return [...this.queue];
  }

  // remove a change from queue (after successful sync)  private removeFromQueue(id: string) {
    this.queue = this.queue.filter(q => q.id !== id);
    this.saveToStorage();
  }

  // process the sync queue  async processQueue(): Promise<void> {
    ...
    // process in order, stop on first failure    for (const change of [...this.queue]) {
      try {
        ...
        // give up after 5 retries        if (change.retryCount >= 5) {
    ...
    // if more changes queued, process again    if (this.queue.length > 0 && this.isOnline) {
      setTimeout(() => this.processQueue(), 5000);
    }
  }

  private async syncChange(change: QueuedChange): Promise<void> {
    // import the singleton api instance directly    const { api } = await import('@/api/nocobase-client');
  ...
  // evict least recently used record when cache is full  private evictLRU() {
    // sort by accessedat ascending, remove oldest    this.cache.records.sort((a, b) => a.accessedAt - b.accessedAt);
    const removed = this.cache.records.shift();
    secureLogger.debug(`[Offline] evicted LRU record ${removed?.collection}/${removed?.id}`);
  }

  // clear all cached data  clearCache() {
    this.cache = { records: [], lastSync: 0, collections: [] };
    this.saveToStorage();
  }

  // clear pending changes (use with caution)  clearQueue() {
    this.queue = [];
    this.saveToStorage();
  }
...
// singleton instanceexport const offlineService = new OfflineService();

// react hook for offline statusexport function useOfflineStatus() {
...
// need to import react for the hookimport React from 'react';
