...
// helper to recursively find files and create nodefssourceasync function walkSources(dir: string, root: string = dir): Promise<NodeFsSource[]> {
...
// helper create a temporary test directory structureasync function makeSampleDir(): Promise<string> {
    const tmp = await fs.promises.mkdtemp(path.join(process.cwd(), 'test-notion-'));
    // create page    const md = `---\ntitle: My Page\nfoo: bar\n---\nThis is content.`;
    await fs.promises.writeFile(path.join(tmp, 'page1.md'), md);
    // nested markdown    await fs.promises.mkdir(path.join(tmp, 'sub'));
    await fs.promises.writeFile(path.join(tmp, 'sub', 'page2.md'), 'simple page');
    // create database csv    const csv = 'Name,Value\nAlice,10\nBob,20';
    await fs.promises.writeFile(path.join(tmp, 'db.csv'), csv);
    // another csv nested    await fs.promises.writeFile(path.join(tmp, 'sub', 'db2.csv'), 'X,Y\n1,2');
    // asset subfolder    await fs.promises.mkdir(path.join(tmp, 'assets'));
...
describe('Notion parser', () => {
    it('should read pages, csvs and assets', async () => {
        ...
        // two pages from root and nested folder        expect(ws.pages.length).toBe(2);
        ...
        // databases should include db and db2        expect(ws.databases.length).toBe(2);
