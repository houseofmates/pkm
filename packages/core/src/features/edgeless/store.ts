...
async function flushPendingOps(drawingId: string) {
  ...
  // flush in bounded batches to avoid blocking the main thread  const batch = ops.splice(0, OPLOG_FLUSH_BATCH_SIZE)

  try {
    await appendOps(drawingId, batch)
  } catch (e) {
    secureLogger.error('[oplog] flush failed', e)
    // keep ops in memory, so we can retry    ops.unshift(...batch)
  } finally {
    if (ops.length > 0) {
...
// oplog-based history state (replaces json snapshot stack)export interface OplogHistory {
  ops: string[] // oplog entry ids
  undone: string[] // ids of undone ops for redo
}

interface EdgelessState {
  // canvas identity  drawingId: string
  setDrawingId: (id: string) => void

  // fabric canvas reference (shared)  fabricCanvas: fabric.Canvas | null
  setFabricCanvas: (c: fabric.Canvas | null) => void

  // selected objects (ids)  selectedIds: Set<string>
  setSelectedIds: (ids: Set<string>) => void

  activeElementId: string | null
  setActiveElementId: (id: string | null) => void

  // pdf document  pdfDoc: unknown | null
  setPdfDoc: (doc: unknown | null) => void

  // layers  layers: EdgelessLayer[]
  activeLayerId: string

  // elements (non-canvas html overlays)  elements: EdgelessElement[]
  // o(1) lookup map kept in sync with elements array  elementMap: Map<string, EdgelessElement>

  // viewport  viewPort: { x: number; y: number; zoom: number }

  // interaction mode  mode: 'interact' | 'draw'
  activeTool: ToolType
  ...
  // spatial index for fast lookup  spatialIndex: SpatialIndex | null

  // history (oplog-based)  history: OplogHistory

  // ui state  selectionMode: 'cursor' | 'free' | 'rect' | 'magic' | 'grab'
  eraserWidth: number
  textSize: number
  isChatOpen: boolean
  isLinking: boolean
  ...
  // actions  addLayer: (name: string) => void
  ...
  // history actions (oplog-based)  recordOp: (op: DrawOp) => Promise<void>
  undo: () => Promise<void>
  redo: () => Promise<void>
  loadFromOplog: (drawingId: string) => Promise<void>

  // spatial index actions  setSpatialIndex: (index: SpatialIndex | null) => void
  rebuildSpatialIndex: (canvas: fabric.Canvas | null) => void

  // ui actions  setSelectionMode: (mode: 'cursor' | 'free' | 'rect' | 'magic' | 'grab') => void
...
// ─── helpers ────────────────────────────────────────────────────────────────────
/** build a map<id, element> from an array – used by every mutation */function buildMap(elements: EdgelessElement[]): Map<string, EdgelessElement> {
...
export const useEdgelessStore = create<EdgelessState>()((set, get) => ({
  ...
  setDrawingId: (id) => {
    set({ drawingId: id })
    // start sync for this drawing    canvasSync.start()
  },

  // layers  layers: [{ id: 'default', name: 'layer 1', visible: true, locked: false }],
  ...
  // config  canvasConfig: { mode: 'edgeless' },

  // spatial index  spatialIndex: null,

  // history (empty oplog)  history: { ops: [], undone: [] },

  // ui defaults  selectionMode: 'grab',
  eraserWidth: 20,
  textSize: 40,
  isChatOpen: false,
  isLinking: false,
  ...
  // canvas refs and helpers  fabricCanvas: null,
  selectedIds: new Set(),
  activeElementId: null,
  pdfDoc: null,

  // layer actions  addLayer: (name) =>
    set((state) => {
  ...
  // element actions ─── all keep elementmap in sync ──────────────────────────  addElement: (el) =>
    set((state) => {
  ...
  updateElement: (id, patch) =>
    set((state) => {
      ...
      // only create a new array if something actually changed      if (updated === existing) return state
      const newMap = new Map(state.elementMap)
  ...
  // mode actions  setMode: (mode) => set({ mode }),
  ...
  // oplog-based history  recordOp: async (op) => {
    const { drawingId, history, activeLayerId } = get()
    if (!drawingId) return

    // apply strong typing/copy to avoid mutation from outside    const opWithLayer: DrawOp = {
      ...op,
    ...
    // optimistic ui: update history immediately    set((state) => ({
      history: {
    ...
    // buffer for batched persistence    const buffer = pendingOpsByDrawing.get(drawingId) ?? []
    if (!pendingOpsByDrawing.has(drawingId)) pendingOpsByDrawing.set(drawingId, buffer)
    buffer.push(entry)
    scheduleFlush(drawingId)

    // mark as pending in metadata (async, non-blocking)    void updateDrawingMeta(drawingId, { syncState: 'pending' }).catch((e) => {
  ...
  undo: async () => {
    ...
    // move last op to undone stack    const lastId = history.ops[history.ops.length - 1]
    ...
    // reload state from oplog minus the undone op    // (in a full implementation, we'd compute inverse ops or reload from checkpoint)    secureLogger.debug('undo:', lastId)

    // save checkpoint after significant undo    if (remaining.length % 20 === 0) {
  ...
  loadFromOplog: async (drawingId) => {
    // load checkpoint + recent ops    const checkpoint = await getLatestCheckpoint(drawingId)
    ...
    // set drawing id    set({ drawingId })

    // emit event for canvas to load    window.dispatchEvent(
  ...
  // spatial index  setSpatialIndex: (index) => set({ spatialIndex: index }),
  ...
  // ui actions  setSelectionMode: (mode) => set({ selectionMode: mode }),
  ...
  // new canvas helpers  setFabricCanvas: (c) => set({ fabricCanvas: c }),
...
// ─── granular selectors ─────────────────────────────────────────────────────────// these hooks subscribe to a single slice of state, preventing cascading re-renders.
/** subscribe to a single element by id. returns undefined if not found. *  the component only re-renders when *this specific element object* changes. */
export function useElement(id: string): EdgelessElement | undefined {
  return useEdgelessStore((s) => s.elementMap.get(id))
}

/** subscribe to the viewport with shallow equality – prevents re-render *  when the reference changes but x/y/zoom values are identical. */
export function useViewport() {
  return useEdgelessStore(useShallow((s: EdgelessState) => s.viewPort))
}

/** subscribe to the element ids array only (not the element objects). *  re-renders only when elements are added/removed, not when one is moved. */
...
/** subscribe to the active tool */export function useActiveTool(): ToolType {
  return useEdgelessStore((s) => s.activeTool)
}

/** subscribe to selection mode */export function useSelectionMode() {
  return useEdgelessStore((s) => s.selectionMode)
}

// import for meta updateimport { updateDrawingMeta } from './storage/canvas-db'
