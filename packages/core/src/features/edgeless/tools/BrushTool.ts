...
export class BrushTool extends BaseTool {
  ...
  // dynamics  public streamline = 0;     // 0-100
  ...
  onStart(ctx: ToolContext, x: number, y: number, pressure: number) {
    ...
    // draw the initial dot    this.drawDot(ctx, point);
  }
  ...
  onMove(ctx: ToolContext, x: number, y: number, pressure: number) {
    ...
    // 1. add raw point to engine to get smoothed point    // we don't use the raw (x,y) for drawing directly, we use the smoothed result    const point = this.engine.addPoint({ x, y, pressure: mappedPressure });

    // 2. interpolate between lastsmoothedpoint and new smoothed point    const dx = point.x - this.lastSmoothedPoint.x;
    const dy = point.y - this.lastSmoothedPoint.y;
    const distance = Math.sqrt(dx * dx + dy * dy);

    // calculate step size based on brush size and spacing    // we use the pressure-adjusted size at the *start* of the segment to approximate    const startSize = this.size * this.lastSmoothedPoint.pressure;
    const step = Math.max(1, startSize * this.spacing);

    // 3. fill gaps    let currentDist = this.leftoverDistance;
  ...
  private drawDot(ctx: ToolContext, point: Point) {
    ...
    // calculate effective size with pressure and jitter    let effectiveSize = this.size * point.pressure;
    if (this.sizeJitter > 0) {
      effectiveSize *= 1 + (Math.random() - 0.5) * (this.sizeJitter / 50);
    }

    // scatter    let finalX = point.x;
    let finalY = point.y;
    if (this.scatter > 0) {
    ...
    // blend    if (this.blend > 0) {
      layerCtx.globalAlpha = 1 - (this.blend / 200);
    }

    layerCtx.beginPath();
    // draw a circle for the dab    layerCtx.arc(finalX, finalY, effectiveSize / 2, 0, Math.PI * 2);
  ...
  /** map input pressure through cubic bezier curve */  private applyPressureCurve(pressure: number): number {
    const [x1, y1, x2, y2] = this.pressureCurve;
    // simple cubic bezier approximation (t = pressure)    const t = Math.max(0, Math.min(1, pressure));
    const mt = 1 - t;
