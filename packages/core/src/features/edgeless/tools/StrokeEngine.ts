...
export class StrokeEngine {
  ...
  public setPressureCurve(points: [number, number][]) {
    // basic linear interpolation for the curve points    this.pressureCurve = (p) => {
      if (points.length < 2) return p;
      // simple implementation: find segment and interpolate      for (let i = 0; i < points.length - 1; i++) {
  ...
  private applySmoothing(point: Point): Point {
    // if we have at least two prior points, use a small 3-point weighted average (better curvature and pressure smoothing)    if (this.points.length >= 2) {
      const a = this.points[this.points.length - 2];
      const b = this.points[this.points.length - 1];
      // weighted average (gives more weight to the most recent point)      return {
    ...
    // fallback to the original exponential smoothing for the very first sample    return {
