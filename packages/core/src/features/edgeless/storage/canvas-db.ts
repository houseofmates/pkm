...
let dbInstance: IDBPDatabase<CanvasDBSchema> | null = null;
 ...
 */
export async function getUnsyncedOps(
  drawingId: string,
  limit = Infinity,
): Promise<OpLogEntry[]> {
  const db = await getCanvasDB();
...
export async function pruneOldOps(
  drawingId: string,
  keepCount = 500,
): Promise<number> {
  try {
    const db = await getCanvasDB();
    const index = db
      .transaction("oplog", "readwrite")
      .store.index("by-drawing");
    const range = IDBKeyRange.only(drawingId);
...
export async function getLatestCheckpoint(
  drawingId: string,
): Promise<CanvasCheckpoint | undefined> {
  const db = await getCanvasDB();
  const all = await db.getAllFromIndex("checkpoints", "by-drawing", drawingId);
