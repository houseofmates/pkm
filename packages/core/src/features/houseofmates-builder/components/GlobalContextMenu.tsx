...
export function GlobalContextMenu({ x, y, onClose }: Props) {
  ...
  const handleBackgroundImage = () => {
    ...
    if (url !== null) {
      if (url.trim()) {
        updatePage({
          background: `url("${url.trim()}") center center / cover no-repeat fixed`,
        });
  ...
  const handleUploadBackground = () => {
    ...
    input.onchange = async (e: any) => {
      ...
      securelogger.info(
        "[globalcontextmenu] uploading file:",
        file.name,
        file.type,
        file.size,
      );
      ...
      try {
        ...
        if (url) {
          ...
          securelogger.error(
            "[globalcontextmenu] no url found in upload response:",
            uploaded,
          );
        ...
        securelogger.error(
          "[globalcontextmenu] error details:",
          err?.response || err?.message,
        );
        toast.error(
          "failed to upload background: " + (err?.message || "unknown error"),
        );
      }
      onclose();
    };
  ...
  const getdefaultcontent = (type: string) => {
    switch (type) {
      ...
      case "image":
        return { url: "", alt: "" };
      case "video":
        return { url: "", autoplay: false, loop: false, muted: true };
      case "embed":
        return { url: "" };
      case "shape":
        return { fill: "#ffffff" };
  ...
  const handledatabaseviewselect = (
  ...
  if (showColorPicker) {
    return (
      ...
      >
        <div
          classname="bg-[#050505] border border-white/10 rounded-2xl p-6 w-80"
          onclick={(e) => e.stoppropagation()}
        >
          <div classname="flex justify-between items-center mb-4">
            <h3 classname="text-lg font-bold text-[var(--primary)] lowercase">
              background color
            </h3>
            <button
              onclick={onclose}
  ...
  if (showpagemodal) {
    ...
    return (
      ...
      >
        <div
          classname="bg-[#050505] border border-white/10 rounded-2xl p-6 w-96"
          onclick={(e) => e.stoppropagation()}
        >
          <div classname="flex justify-between items-center mb-4">
            <h3 classname="text-lg font-bold text-[var(--primary)] lowercase">
          ...
          <label classname="block text-white/60 text-sm mb-2 lowercase">
            page title
          </label>
          <input
            type="text"
          ...
          {newpagetitle && (
            <div classname="mb-4 p-3 rounded-xl bg-white/5 border border-white/10">
              <p classname="text-xs text-white/40 lowercase mb-1">
                url preview
              </p>
              <p classname="text-sm text-[var(--primary)] font-mono">
                {import.meta.env.vite_domain || "localhost"}/
                <span classname="text-white">{previewslug || "page-slug"}</span>
              </p>
            </div>
          )}
          <button
      ...
      >
        ...
        <div className="relative">
          ...
          >
            ...
            <ChevronRight
              className={`w-4 h-4 transition-transform ${showBackgroundMenu ? "rotate-90" : ""}`}
            />
          </button>
        ...
        <div className="relative">
          <button
            onClick={() => {
              ...
              toast.success(
                `page sounds ${!page?.enable_sounds ? "enabled" : "disabled"}`,
              );
              onClose();
            }}
            className="w-full px-3 py-2 flex items-center justify-between text-white/80 hover:bg-white/10 transition-colors lowercase"
          >
            ...
            <span
              className={`text-[10px] px-1.5 py-0.5 rounded ${page?.enable_sounds ? "bg-[var(--primary)]/20 text-[var(--primary)]" : "bg-white/10 text-white/40"}`}
            >
        ...
        <div className="relative">
          ...
          >
            <span className="flex items-center gap-3">
              <ChevronRight
                className={`w-4 h-4 transition-transform ${showAddMenu ? "rotate-90" : ""}`}
              />
              insert at cursor
            </span>
          </button>
          ...
          {showAddMenu && (
            <div className="pl-6 border-l border-white/10 ml-3">
              <button
                onClick={() => addElementAtCursor("text")}
                className="w-full px-3 py-2 flex items-center gap-3 text-white/60 hover:bg-white/10 hover:text-white lowercase"
              >
                <Type className="w-4 h-4" /> text
              </button>
              <button
                onClick={() => addElementAtCursor("button")}
                className="w-full px-3 py-2 flex items-center gap-3 text-white/60 hover:bg-white/10 hover:text-white lowercase"
              >
                <MousePointerClick className="w-4 h-4" /> button
              </button>
              <button
                onClick={() => addElementAtCursor("image")}
                className="w-full px-3 py-2 flex items-center gap-3 text-white/60 hover:bg-white/10 hover:text-white lowercase"
              >
                <Image className="w-4 h-4" /> image
              </button>
              <button
                onClick={() => addElementAtCursor("video")}
                className="w-full px-3 py-2 flex items-center gap-3 text-white/60 hover:bg-white/10 hover:text-white lowercase"
              >
                <Play className="w-4 h-4" /> video
              </button>
              <button
                onClick={() => addElementAtCursor("embed")}
                className="w-full px-3 py-2 flex items-center gap-3 text-white/60 hover:bg-white/10 hover:text-white lowercase"
              >
                <Code className="w-4 h-4" /> embed
              </button>
              <button
                onClick={() => addElementAtCursor("shape")}
                className="w-full px-3 py-2 flex items-center gap-3 text-white/60 hover:bg-white/10 hover:text-white lowercase"
              >
              ...
              <button
                ...
                className="w-full px-3 py-2 flex items-center gap-3 text-[var(--primary)] hover:bg-white/10 lowercase"
              >
