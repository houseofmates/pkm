...
export const HeadmateCard = React.memo(forwardRef<HTMLDivElement, HeadmateCardProps & React.HTMLAttributes<HTMLDivElement>>(({ member, collection, onClick, className, ...props }, ref) => {
  ...
  // fallback to member.name if rawname is missing or looks like an id  const isId = (val: any) => {
    const s = String(val);
    if (!s) return true;
    // numeric ids    if (/^\d+$/.test(s)) return true;
    // simplyplural / pluralkit style alphanumeric ids (often long, or member_ prefixes)    if (s.length > 15 && /^[a-zA-Z0-9_\-]+$/.test(s)) return true;
