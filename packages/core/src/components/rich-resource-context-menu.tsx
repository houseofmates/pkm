...
export function RichResourceContextMenuContent({ currentName, currentColor, onUpdate, onHide, children, itemId }: RichResourceContextMenuProps) {
  return (
    <ErrorBoundary fallback={<div className="text-red-500">menu failed</div>}>
      {renderMenu({ currentName, currentColor, onUpdate, onHide, children, itemId })}[<source_file_content>{
        "path": "/home/house/pkm/packages/core/src/components/rich-resource-context-menu.tsx",
        "content": "export function RichResourceContextMenuContent({currentName, currentColor, onUpdate, onHide, children, itemId}: RichResourceContextMenuProps) {\n  return (\n    <ErrorBoundary fallback={<div className=\"text-red-500\">menu failed</div>}>\n      {renderMenu({ currentName, currentColor, onUpdate, onHide, children, itemId })}[<function_calls>\n<invoke name=\"edit\">\n<parameter name=\"explanation\">Add hide button to context menu children section</parameter>\n<parameter name=\"file_path\">/home/house/pkm/packages/core/src/components/rich-resource-context-menu.tsx</parameter>\n < parameter name =\"old_string\">      {/* footer: upload */}\n      <div className=\"p-3 border-t bg-muted/30 shrink-0\">\n        <Button\n          variant=\"outline\"\n          className=\"w-full h-10 text-sm font-medium border-dashed border-muted-foreground/30 hover:bg-muted hover:text-foreground transition-all rounded-xl\"\n          onClick={() => document.getElementById('icon-upload')?.click()}\n        >\n          <Upload className=\"h-4 w-4 mr-2\" /> upload custom icon\n        </Button>\n        {children && (\n          <div className=\"mt-2 pt-2 border-t border-border/50 flex flex-col gap-1\">\n            {children}\n          </div>\n        )}</parameter>\n<parameter name=\"new_string\">      {/* footer: upload */}\n      <div className=\"p-3 border-t bg-muted/30 shrink-0\">\n        <Button\n          variant=\"outline\"\n          className=\"w-full h-10 text-sm font-medium border-dashed border-muted-foreground/30 hover:bg-muted hover:text-foreground transition-all rounded-xl\"\n          onClick={() => document.getElementById('icon-upload')?.click()}\n        >\n          <Upload className=\"h-4 w-4 mr-2\" /> upload custom icon\n        </Button>\n        {onHide && (\n          <Button\n            variant=\"ghost\"\n            className=\"w-full h-10 text-sm font-medium hover:bg-muted hover:text-foreground transition-all rounded-xl mt-2\"\n            onClick={onHide}\n          >\n            <EyeOff className=\"h-4 w-4 mr-2\" /> hide from sidebar\n          </Button>\n        )}\n        {children && (\n          <div className=\"mt-2 pt-2 border-t border-border/50 flex flex-col gap-1\">\n            {children}\n          </div>\n        )}</parameter>\n</invoke>\n</function_calls>]}\n</invoke>\n</function_calls>
    </ErrorBoundary >
  );
}
