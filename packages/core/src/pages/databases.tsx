...
export function DatabasesPage({ onSelect }: DatabasesPageProps) {
  const { isAuthenticated, loginWithApiKey, token } = useAuth();
  ...
  const FALLBACK_FIELDS: Record<
    string,
    Array<{ name: string; type: string; interface?: string }>
  > = {
    events: [
  ...
  const collectionsWithFallbackFields = filteredCollections.map(
    (col: Collection) => {
      ...
      await Promise.all(
        collectionsWithFallbackFields.map(async (col) => {
          try {
            ...
            secureLogger.warn(
              `[Databases] Failed to fetch count for ${col.name}:`,
              err,
            );
            counts[col.name] = 0;
          }
  ...
  useEffect(() => {
    const allowed =
      (location.state as any)?.fromSidebar ||
      storageManager.getItem("pkm:allow_databases_direct");
  ...
  const sidebarDocs = sidebarItems
    .filter(
      (item) => item.id.startsWith("doc_") || item.id.startsWith("drawing_"),
    )
    .map((item) => ({
  ...
  const handleDragEnd = (event: any) => {
    ...
    if (active.id !== over?.id) {
      ...
      const newOrder = arrayMove(
        sortedCollections.map((c) => c.name),
        oldIndex,
        newIndex,
      );
      setDbOrder(newOrder);
  ...
  const handleSelect = (name: string) => {
    if (onSelect) {
      ...
      if (name.startsWith("doc_")) {
        navigate(`/canvas/${name.replace("doc_", "")}`, {
          state: { fromSidebar: true },
        });
        return;
      }
      if (name.startsWith("drawing_")) {
        navigate(`/drawings/${name.replace("drawing_", "")}`, {
          state: { fromSidebar: true },
        });
        return;
      }
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      toast.error(
        "failed to connect to nocobase. please check your api token.",
      );
    } finally {
      setValidating(false);
    }
  ...
  if (error) {
    return (
      <div className="p-8 text-destructive lowercase">
  ...
  if (loading && collections.length === 0) {
    return (
      <div className="p-8 text-muted-foreground lowercase">
        loading databases...
      </div>
    );
  }
