// ai.worker.ts — dedicated web worker for all heavy ai/vector operations
// runs embedding generation, knowledge-base search, rag context building,
// and llm streaming off the main thread.
//
// exposed via comlink so the main thread calls typed async methods.

import * as Comlink from 'comlink';
import type { AIWorkerAPI, SearchResultDTO, RagPromptResult, AskWithRagResult } from './ai-worker-types';

// ---------------------------------------------------------------------------
// configuration (mirrored from vector-store / llm-config, but self-contained)
// ---------------------------------------------------------------------------
...
// the worker receives api base url + auth token at init time so it can call
// nocobase endpoints without sharing the main-thread axios singleton.
let _apiBaseUrl = '';
let _authToken = '';

/** called once from the main thread right after Comlink.wrap */
export function init(apiBaseUrl: string, authToken: string, vectorConfig?: Partial<typeof VECTOR_CONFIG>) {
...
// ---------------------------------------------------------------------------
// internal helpers — fetch wrappers for worker context
// ---------------------------------------------------------------------------
...
async function generateEmbedding(text: string): Promise<number[]> {
    const res = await fetch(VECTOR_CONFIG.embeddingEndpoint, {
        method: 'POST',
...
// ---------------------------------------------------------------------------
// text chunking (same logic as vector-store.ts)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// rag context building (mirrors rag-service.ts)
// ---------------------------------------------------------------------------
...
// wilson rag prompt template (inlined from rag-prompts.ts to avoid import issues)
...
// ---------------------------------------------------------------------------
// llm streaming (ported from ollama-client.ts — pure fetch, no DOM)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// extract source references from a prompt string
// ---------------------------------------------------------------------------
...
const workerAPI: AIWorkerAPI & { init: typeof init } = {
    init,
    searchKnowledgeBase,
    generateEmbedding,
    buildRagPrompt,
