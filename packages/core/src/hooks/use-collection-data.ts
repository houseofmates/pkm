...
interface CollectionClient {
  ...
  ): Promise<{ data: SchemaRecord[] }>;
  createRecord(
    name: string,
    data: Record<string, unknown>,
  ): Promise<{ id?: string | number; data?: { id?: string | number } }>;
  updateRecord(
    name: string,
    id: string | number,
    data: Record<string, unknown>,
...
export function useCollectionData(
  ...
  const { collections: availableCollections, loading: collectionsLoading } =
    useCollections();
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (!colData?.fields) {
        try {
          secureLogger.info(
            "attempting to fetch full collection schema with fields...",
          );
          ...
          if (e?.response?.status === 404) {
            secureLogger.warn(
              `collection "${collectionName}" not found (404), skipping`,
            );
            if (fetchId === fetchCounterRef.current) {
              setFetchError(`collection "${collectionName}" not found`);
              setLoading(false);
            }
            return;
          }
          secureLogger.warn(
            "failed to fetch collection schema, using preloaded if available",
            e,
          );
      ...
      if (colData && fetchId === fetchCounterRef.current) {
        ...
        let hasFronter = colData.fields?.some(
        ...
        if (!hasFronter && fetchId === fetchCounterRef.current) {
          try {
            secureLogger.info(
              'auto-creating "fronter" field for',
              collectionName,
            );
    ...
    async (record: SchemaRecord) => {
      ...
      if (!record || record.id === undefined || record.id === null) {
        if (recordWithTs.timestamp) {
          try {
            await client.deleteRecordByFilter(collectionName, {
              timestamp: recordWithTs.timestamp,
            });
            ...
            setRecords((prev) =>
              prev.filter(
                (r) =>
                  (r as RecordWithTimestamp).timestamp !==
                  recordWithTs.timestamp,
              ),
            );
            return;
          } catch (error) {
    ...
    async (recordToRestore: SchemaRecord) => {
      try {
        const {
          ...
          updated_at: _updated_at,
          ...rest
        } = recordToRestore as SchemaRecord & {
          created_at?: unknown;
          updated_at?: unknown;
        };
        await client.createRecord(
        ...
        fetchData();
        setDeletedStack((prev) =>
          prev.filter((r) => r.id !== recordToRestore.id),
        );
      } catch (e) {
        secureLogger.error("failed to undo delete", e);
        toast.error("failed to undo delete");
  ...
  useEffect(() => {
    const handleCreate = async (evt: Event) => {
      ...
      if (e.detail?.collection === collectionName) {
        try {
          const createRes = await client.createRecord(
            collectionName,
            e.detail.data || {},
          );
          ...
          if (
            newId &&
            collection?.fields?.some((f: FieldDefinition) => f.name === "ai")
          ) {
            generateAndSaveAiField(collectionName, newId, "ai", {
              instruction:
                "provide a brief summary and initial ideas for this new entry",
              topK: 5,
            }).catch((err) =>
              secureLogger.warn("auto-suggest generation failed", err),
            );
          }
        } catch (err) {
