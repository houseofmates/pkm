// use-sidebar-colors.ts// hook for syncing sidebar item colors across all devices via nocobase
...
// local cache key prefixconst CACHE_KEY_PREFIX = 'pkm_sidebar_color:';
...
export function useSidebarColors(options?: { pollIntervalMs?: number }) {
  ...
  // ensure collection exists on mount  useEffect(() => {
    if (isAuthenticated) {
  ...
  // fetch colors from server  const fetchColors = useCallback(async (): Promise<SidebarColorMap> => {
  ...
  // use react-query for server state  const { data: serverColors = {}, isLoading, error } = useQuery({
  ...
  // merge server colors with local colors (server wins)  useEffect(() => {
    setLocalColors(prev => ({
      ...prev,
      ...serverColors
    }));
  ...
  // subscribe to changes from other tabs  useEffect(() => {
    const unsubscribe = subscribeToColorChanges((itemId, color) => {
      ...
      // invalidate query to refetch from server      queryClient.invalidateQueries({ queryKey: ['sidebar-colors'] });
    });
    
    return unsubscribe;
  }, [queryClient]);

  // mutation for updating a color  const updateColorMutation = useMutation({
    mutationFn: async ({
      itemId,
      itemType,
      color
      ...
      // optimistic update      setLocalColors(prev => ({
        ...prev,
        [itemId]: { ...prev[itemId], color }
      }));
      ...
      // sync to server      const success = await updateSidebarItemColor(itemId, itemType, color);
    ...
    onSuccess: () => {
      // refetch to ensure consistency      queryClient.invalidateQueries({ queryKey: ['sidebar-colors'] });
    },
    onError: (error, variables) => {
      secureLogger.error('[use-sidebar-colors] update failed:', error);
      // revert optimistic update on error      setLocalColors(prev => {
  ...
  // mutation for updating full metadata (color + icon)  const updateMetadataMutation = useMutation({
    mutationFn: async ({
      itemId,
      itemType,
      metadata
      ...
      // sync to server      const success = await saveSidebarColor(itemId, itemType, metadata);
    ...
    onError: (error, variables) => {
      ...
      // revert on error      setLocalColors(prev => {
        const { [variables.itemId]: _, ...rest } = prev;
        return rest;
      });
    }
  ...
  // mutation for deleting a color  const deleteColorMutation = useMutation({
    ...
    onSuccess: (itemId) => {
      // remove from local state      setLocalColors(prev => {
        const { [itemId]: _, ...rest } = prev;
        return rest;
      });
      
      // remove from cache      try {
  ...
  // helper to update just color (auto-detects item type)  const updateColor = useCallback(
  ...
  // helper to update full metadata  const updateMetadata = useCallback(
  ...
  // helper to get color for an item (with fallback)  const getColor = useCallback(
  ...
  // helper to get full metadata for an item  const getMetadata = useCallback(
  ...
  // refetch function  const refetch = useCallback(() => {
    return queryClient.invalidateQueries({ queryKey: ['sidebar-colors'] });
  }, [queryClient]);

  return {
    // state    colors: localColors,
    isLoading,
    error,
    ...
    // mutation states    isUpdating: updateColorMutation.isPending || updateMetadataMutation.isPending,
