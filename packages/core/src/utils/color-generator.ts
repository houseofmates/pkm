...
// simple hash function to generate a consistent number from a stringfunction hashCode(str: string): number {
  let hash = 0;
...
export function getStringColor(str: string): string {
  ...
  // generate hue: 0-360  const hue = hash % 360;
  // generate saturation: 60-90% for vibrancy  const saturation = 60 + (hash % 30);
  // generate lightness: 45-65% for readability  const lightness = 45 + (hash % 20);
...
export function getOutlineColorFromHsl(hue: number, saturation: number, lightness: number): string {
  // "darker": reduce lightness significantly (keep it readable but dark)  // user requested "25% darker" -> increase reduction from 40 to 50  const newL = Math.max(5, lightness - 50);

  // "more vibrant": increase saturation  const newS = Math.min(100, saturation + 20);

  // "base color only": no hue shift (user said "not blue... just darker base")  const newH = hue;
...
export function getColorStyles(str: string) {
  ...
  // helper for css text-stroke  // use the calculated specific darker/vibrant outline  WebkitTextStroke: `3.5px ${outline}`,
