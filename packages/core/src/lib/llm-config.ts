// helper utilities for resolving the llm endpoint (google gemini)// - prefer a runtime user setting stored in localstorage ('gemini_api_url', fallback to 'wilson_api_url')// - storagemanager wrapper used for safer access
...
// - fall back to vite_gemini_url / vite_ollama_url if provided at build time// - default to the google gemini public api endpoint when nothing else is configured// all helpers return normalized urls (no trailing slashes) or full endpoint urls
export function normalizeBaseUrl(urlOrEndpoint?: string): string {
  if (!urlOrEndpoint) return '';
  let u = String(urlOrEndpoint).trim();
  // strip trailing slashes  u = u.replace(/\/+$/, '');
  // remove common api suffixes if present  u = u.replace(/(\/api\/generate|\/api\/chat|\/api)$/i, '');
  return u;
}
...
export function getOllamaBase(): string {
  ...
  // default to local ollama at the user's ip  return DEFAULT_OLLAMA_URL;
}
