...
export function BacklinksFooter({ recordId, collectionName }: { recordId: string, collectionName: string }) {
    ...
    return (
        <div className="mt-12 pt-6 border-t border-border">
            <h3 className="text-sm font-bold text-muted-foreground mb-4 uppercase tracking-wider">Linked Mentions</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {backlinks.map(link => (
                    ...
                    >
                        ...
                        <div className="text-xs text-muted-foreground capitalize">{link.collection}</div>
                    </Link>
                ))}
            </div>
        </div>
