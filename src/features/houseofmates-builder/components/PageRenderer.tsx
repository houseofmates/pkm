...
import { FormRenderer, type FormElementData } from './FormRenderer';
    ...
    useEffect(() => {
        ...
        const handleGlobalMousedown = (e: MouseEvent) => {
            ...
            if (!isModifier && !isClickingElement && !isClickingHandle && !isClickingBubbleMenu && !isClickingModal) {
                console.log('[PageRenderer] Global Deselection Triggered (No Modifier)');
                setSelectedElementIds([]);
...
function ElementRenderer({ element, isSelected, isAdmin, onSelect, onUpdate, onUpdateBatch, onContextMenu }: ElementRendererProps) {
    ...
    console.log(`[ElementRenderer] RENDERING: ${element.type} (${element.id}) at ${posX},${posY}`);
                    ...
                    if (el.type === 'text' && dom) {
                        ...
                        if (richText) {
                            ...
                            if (actualHeight > 0 && Math.abs(actualHeight - elLayout.height) > 5) {
                                console.log(`[PageRenderer] Auto-fitting text height for ${el.id}: ${elLayout.height} -> ${actualHeight}`);
