...
export interface ElementData {
    id: string;
    type: 'text' | 'image' | 'button' | 'container' | 'video' | 'embed' | 'shape' |
...
export function HouseofmatesBuilder() {
    ...
    useEffect(() => {
        ...
        if (key && key !== 'null' && key !== 'undefined') {
            setIsAdmin(true);
            // Run collection ensures in background without blocking page load
            Promise.all([
    ...
    const ensureWebsiteCollection = async () => {
        ...
        try {
            ...
            console.log(`[ensureWebsiteCollection] Collections response:`, collections);
            const col = collections.data?.find((c: any) => c.name === colName);
            console.log(`[ensureWebsiteCollection] Found ${colName}:`, !!col);
            ...
            if (!col) {
                ...
                console.log(`[ensureWebsiteCollection] Create result:`, createRes);
            } else {
                // Upsert fields
                const colDetail = await api.getCollection(colName);
                // Ensure metadata (title/hidden) normalized
                try {
                    await api.updateCollection(colName, { title: colTitle, hidden: true });
                } catch (e) {
                    console.warn(`Failed to normalize collection metadata for ${colName}:`, e);
                }
                // Log existing fields for debugging
                console.log(`${colName} collection details:`, colDetail);
                const existingFields = colDetail.data?.fields || [];
                console.log(`Existing ${colName} fields:`, existingFields.map((f: any) => f.name));

                for (const field of fields) {
                    const existing = existingFields.find((ef: any) => ef.name === field.name);
                    if (!existing) {
    ...
    const updateElements = useCallback((batchUpdates: { id: string; updates: Partial<ElementData> }[]) => {
        ...
        api.updateRecord(collectionNames.website, page.id, { elements: JSON.stringify(newElements) })
            .catch(console.error);
    }, [page, collectionNames, previewMode]);
